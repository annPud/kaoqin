/**
 * Created by ____′↘夏悸，Universe，風亦飛 User: ____′↘夏悸 Date: 2012-8-8 这个是一个jQuery
 * Easyui的的桌面扩展 讨论学习群： 142872541（一） 47729185（二） 70168958（500人超级群）
 * 腾讯微博群号：83952631 社区： http://bbs.btboys.com version: 0.4
 */
(function($) {
	var loaded = false;
	var prevOpenedApp, currentOpenedApp, allWindow = {};
	/**
	 * layout初始化
	 * 
	 * @param target
	...
	function initTaskBlank(target) {
		...
		if (opts.taskBlankPos == 'south' || opts.taskBlankPos == 'north') {
			...
			var scrollLeft = $('<div/>').addClass('app-scroll-left')
					.appendTo(taskList);
			var scrollRight = $('<div/>').addClass('app-scroll-right')
					.appendTo(taskList);
			var listWrap = $('<div/>').addClass('app-list-wrap')
					.appendTo(taskList);
			...
			var scrollTop = $('<div/>').addClass('app-scroll-top')
					.appendTo(taskList);
			var scrollBottom = $('<div/>').addClass('app-scroll-bottom')
					.appendTo(taskList);
		}
		...
		taskList.bind('contextmenu', function(e) {
					if (e.target.nodeName == "LI") {
						opts.onTaskBlankContextMenu.call(target, e, $(e.target)
										.attr('l_id'));
					} else {
		...
		if (opts.loadUrl.app && !loaded) {
			$.ajax({
						...
						error : function(XMLHttpRequest, textStatus,
								errorThrown) {
							$.messager.alert("", textStatus || errorThrown,
									"error");
						}
					});
		}
	...
	function initApp(target, apps) {
		...
		var lineAppBlank = ((wall.height() - 20) - (opts.iconSize + 45) * lines)
				/ lines; // 行间隔高度
		var columnAppBlank = ((wall.width() - 20) - (opts.iconSize + 45)
				* columns)
				/ columns; // 列间隔宽度
		// 初始值
		var line = 1, col = 1, top = 20, left = 10;
		...
		for (var i = 0; i < apps.length; i++) {
			...
			var icon = $('<img/>').height(opts.iconSize).width(opts.iconSize)
					.attr('src', app.icon).appendTo(appItem);
		...
		appItems.mousedown(function() {
					...
					opts.onAppContextMenu.call(target, e, $(this)
									.attr('app_id'));
					e.preventDefault();
				});
	}
	...
	function initAppDrag(target, appItem) {
		appItem.draggable({
					...
					onDrop : function(e, source) {
						if ($(source).prev().attr('app_id') == $(this)
								.attr('app_id')) {
	...
	function createMenu(target, menus, opt) {
		...
		function appendChild(menu) {
			var itemText = menu.text, children = menu.children;
	...
	function initCalendar(target) {
		...
		jqTarget.click(function(e) {
					...
					if (c != 'app-taskBar-calendar-x'
							&& c != 'app-taskBar-calendar-y'
							&& !$.contains(calendarDiv[0], e.target)) {
						calendarDiv.hide();
					}
				});
	...
	function openApp(target, options) {
		var jqTarget = $(target), opt = $(target).data('app').options, wall = jqTarget
				.layout('panel', 'center'), uuid, appOpt;
		if (options) {
			if (!options['uuid']) {
				uuid = UUID();
				options['uuid'] = uuid;
		...
		var defaultRequiredConfig = {
			...
			cache : true,
			onOpen : function() {
				appendToList($(this).attr('w_id'),
						$.data(this, 'panel').options.title);
				...
				$('li[l_id="' + $(this).attr('w_id') + '"]').attr('status',
						'opened');
			},
			onClose : function() {
			...
			onMaximize : function() {
				$(this).closest('.window').css("z-index",
		...
		var config = $.extend({}, defaultConfig, appOpt.cnf, customOption,
		...
		function appendToList(uuid, text) {
			...
			if ($('li[l_id="' + uuid + '"]', list).length) {
				...
				var item = $('<li/>').attr("l_id", uuid).addClass('selected')
						.text(text).attr('status', 'opened');
				list.append(item);
				item.click(function() {
							if ($(this).attr('status') == 'opened') {
								var currentWin = $('div[w_id="' + uuid + '"]',
										wall);
								var currentWinZindex = parseInt(currentWin
										.parent().css("z-index"))
										+ 1;
								if (currentWinZindex != $.fn.window.defaults.zIndex) {
									currentWin.parent().css("z-index",
											$.fn.window.defaults.zIndex++);
								} else {
	...
	/**
	 * 第一步是初始化layout 第二部初始化任务栏 第三部初始化桌面 第四步初始化开始菜单 第五步初始化时间 第六步初始化widget
	 */
	var initMethods = [initLayout, initTaskBlank, initDeskTop, initStartMenu,
			initCalendar, initWidget];
	...
	function init(target, options) {
		...
		for (var i = 0; i < initMethods.length; i++) {
			...
			progressBar.progressbar({
					...
					((parseInt(i) + 1) / initMethods.length) * 100);
	...
	$.fn.app = function(options, params) {
		...
		return this.each(function() {
					...
					if (state) {
						...
						options = $.extend({}, $.fn.app.defaults,
								$.fn.app.parseOptions, options);
						$.data(this, 'app', {
									options : options
								});
					}
	...
	$.fn.app.methods = {
		...
		seticonsize : function(size) {
			return this.each(function() {
						...
						appContainer.find("em,li").height(size + 45).width(size
								+ 45);
						appReset(this);
					});
		},
	...
	$.fn.app.defaults = {
		...
		onTaskBlankContextMenu : function(event, appid) {
		}, // 任务栏右键事件
		onWallContextMenu : function(event) {
		}, // 桌面右键事件
		onAppContextMenu : function(event, appid) {
		}, // app右键事件
		onBeforeOpenApp : function(appOpt) {
		}, // 打开app之前的事件,可以返回自定义的窗口options
		onLoaded : function() {
		}, // app实例化完成事件
		onOpenApp : function() {
		}, // app打开事件
		...
		onStartMenuClick : function(item) {
		}, // 开始菜单点击事件
		iframeOpen : false,
		loadUrl : { // 远程数据加载路径
		// app : 'desktop.json', //app数据
			app : '../../../desktop/initDesktopJson',
